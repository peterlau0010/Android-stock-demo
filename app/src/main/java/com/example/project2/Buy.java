		...
		if(chkQTY()==true&& chkPrice()==true){
			...
			while(cursor.moveToNext()){
				...
				if(SCode.equals(jsonSCode)){
					...
					tvSuccess.setText("Buy Stock success(update)");
					update=true;
					TradingSuccess=true;
				}
			...
			if(update==false){
				db.execSQL("insert into Stock(PortfolioID,stockCode,stockName,lotSize,quantityOnHand)values("+ String.valueOf(++id) +",'"+ jsonSCode +"','"+jsonSName+"',"+jsonLot+","+ QTY+")");
				tvSuccess.setText("Buy Stock success(insert)");
				TradingSuccess=true;
			}
			...
			if(TradingSuccess){
				...
				db.execSQL("insert into Trading(tradingId,date,time,stockCode,stockName,quantity,price,buySell)values(" + String.valueOf(++id) + ",'"+ Date +"','"+  Time+"','"+ jsonSCode +"','"+jsonSName+"','" + QTY+"'," + Price+",'Buy')");
			}
		}
